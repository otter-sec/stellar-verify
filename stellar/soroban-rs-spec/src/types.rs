...
pub fn generate_struct(spec: &ScSpecUdtStructV0) -> TokenStream {
    let ident = format_ident!("{}", spec.name.to_string());

    if spec.lib.len() > 0 {
        let lib_ident = format_ident!("{}", spec.lib.to_string());
        quote! {
        ...
        .fields
        .iter()
        .all(|f| f.name.to_string().parse::<usize>().is_ok())
    {
        ...
        let fields = spec.fields.iter().map(|f| {
            let f_ident = format_ident!("{}", f.name.to_string());
            let f_type = generate_type_ident(&f.type_);
...
pub fn generate_enum(spec: &ScSpecUdtEnumV0) -> TokenStream {
    ...
    if spec.lib.len() > 0 {
        ...
        let variants = spec.cases.iter().map(|c| {
            let v_ident = format_ident!("{}", c.name.to_string());
...
pub fn generate_type_ident(spec: &ScSpecTypeDef) -> TokenStream {
    match spec {
        ...
        ScSpecTypeDef::Udt(u) => {
            let ident = format_ident!("{}", u.name.to_string());
            quote! { #ident }
        }
